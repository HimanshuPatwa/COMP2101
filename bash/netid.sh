...
# If user lists a specific interface
[ -v interface ] && find_interface "$interface" && exit 0;

for VARIABLE in $( ip -o link show | awk -F': ' '{print $2}' );do	
	[ "$VARIABLE" == "lo" ] || find_interface "$VARIABLE"
done
#####
